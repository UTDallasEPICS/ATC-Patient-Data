...
const employeeProfile = ({students, employee, currentStudent}) => {
  ...
  const handleArchive = async() => {
    const response = await fetch(`${process.env.AUTH0_BASE_URL}/employee/${employee._id}`, {
      method: "DELETE"
    })
...
export const getServerSideProps = async ({ query }) => {
  const allPatientsRes = await fetch(`${process.env.AUTH0_BASE_URL}/patient`);
  const allPatients = await allPatientsRes.json()

  const employeeID = query.id
  const employeeRes = await fetch(`${process.env.AUTH0_BASE_URL}/employee/${employeeID}`);
  const employee = await employeeRes.json()

  const { data: { patients},} = employee
  var currentStudents = []
  for(const patientID of patients) {
    const currentStudentRes = await fetch(`${process.env.AUTH0_BASE_URL}/patient/${patientID}`)
