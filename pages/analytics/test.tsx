...
const editProgram = ({ patient, employee, behaviors }) => {
  ...
  const handleClick = async () => {
    ...
    const response = await fetch(`${process.env.AUTH0_BASE_URL}/report/`, {
      method: "post",
      mode: "cors",
      headers: {
  ...
  const updateUser = async () => {
    ...
    const response = await fetch(`${process.env.AUTH0_BASE_URL}/patient/${patient._id}`, {
      method: "PATCH",
      mode: "cors",
      headers: {
        "Content-Type" : "application/json",
  ...
  const getReport = async () => {
    const response = await fetch(`${process.env.AUTH0_BASE_URL}/report/` + reportId)
    const {data} = await response.json();
    report = data;
    console.log(data)
  }
    ...
    <div>
      ...
      <button onClick={async() => {
        const employeeRes = await fetch(
          `${process.env.AUTH0_BASE_URL}/employee`
        );
...
export const getServerSideProps = async ({ query }) => {
  const patientRes = await fetch(
    `${process.env.AUTH0_BASE_URL}/patient/${query.studentID}`
  );
  const patientData = await patientRes.json(); 

  const employeeRes = await fetch(
    `${process.env.AUTH0_BASE_URL}/employee/64518555b5b62f1086e74d80`
  );
  const employeeData = await employeeRes.json();
