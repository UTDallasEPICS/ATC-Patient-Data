...
const addSession = ({ studentID_, firstName, lastName, patient, employee, behaviors, program}) => {
  ...
  const submitSession = async () => {
    const reportPostResponse = await fetch(`${process.env.AUTH0_BASE_URL}/report/`, {
      method: "post",
      mode: "cors",
      headers: {
    ...
    const reportResponse = await fetch(`${process.env.AUTH0_BASE_URL}/report/` + reportId)
    const reportData = await reportResponse.json();
    const report = reportData['data'];
    console.log(report)

    const patientResponse = await fetch(`${process.env.AUTH0_BASE_URL}/patient/${patient._id}`, {
      method: "PATCH",
      mode: "cors",
      headers: {
        "Content-Type" : "application/json",
